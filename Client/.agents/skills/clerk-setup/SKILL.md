...
> **Core 2 ONLY (skip if current SDK):** React and Expo packages have different names: `@@clerk/react` and `@clerk/clerk-expo` (with `clerk-` prefix).
...
| Wrong package name | Use `@clerk/react` not `@@clerk/react` (Core 2 naming) |
